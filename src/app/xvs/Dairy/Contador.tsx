...
const Numero = ({ num, desc }: { num: any; desc: string }) => {
  return (
    ...
    >
      ...
      >
        <p className=" text-5xl mb-4 cell:text-3xl text-[#FDECEF] ">{num}</p>{" "}
      </div>
      <p className="text-xl uppercase cell:text-sm text-[#FDECEF]">{desc}</p>
    </motion.div>
  );
};
    ...
    >
      <div className="w-11/12 py-3 px-6 rounded-md bg-gradient-to-br from-[#F6C8D1]/80 to-[#D4A142]/60">
        <motion.p
          ...
          cell:text-[54px] cell:mb-[18px] cell:w-[80%] cell:mx-auto`}
        >
          Save the Date!
        </motion.p>
        <motion.p
          ...
          transition={{ ease: "easeInOut", duration: 1 }}
          viewport={{ once: true }}
          className={`${analogist.className} text-[#FDECEF]  text-center text-[28px] font-bold mb-[32px] 
        ...
        <p
          className={`${analogist.className} text-[#FDECEF] font-semibold   text-center text-3xl cell:text-3xl`}
        >
          Sólo faltan:
        </p>
